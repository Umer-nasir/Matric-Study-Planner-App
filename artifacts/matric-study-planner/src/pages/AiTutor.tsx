...
function getInstantTutorReply(message: string, subject: string): string | null {
  ...
  return subject === 'Urdu'
    ? 'وعلیکم السلام! میٹرک کے اردو مضمون سے متعلق اپنا سوال پوچھیے، میں امتحانی انداز میں واضح جواب دوں گا۔'
...
export default function AiTutor() {
  ...
  async function sendMessage(rawMessage: string, attachment = pendingAttachment) {
    ...
    const instantReply = !attachment ? getInstantTutorReply(trimmed, selectedSubject) : null;
    if (instantReply) {
      setTutorChatHistory([
        ...optimisticHistory,
  ...
  async function handleAttachmentChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!isAcceptedFile(file)) {
      setAttachmentError('Unsupported file type. Please upload JPG, PNG, WEBP, PDF, or DOCX.');
      return;
    }
    ...
    <div className="app-shell flex flex-col">
      ...
      >
        <input
          ref={fileInputRef}
          type="file"
          accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp,.pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
          className="hidden"
