...
function normalizePracticeSet(data: unknown): PracticeSet {
  ...
  return {
    mcqs: Array.isArray(candidate.mcqs)
      ? candidate.mcqs.filter((item): item is NonNullable<PracticeSet['mcqs']>[number] => {
          if (!item || typeof item !== 'object') return false;
          const mcq = item as Record<string, unknown>;
          return isText(mcq.question) &&
            Array.isArray(mcq.options) && mcq.options.length === 4 && mcq.options.every(isText) &&
            Number.isInteger(mcq.correctIndex) && Number(mcq.correctIndex) >= 0 && Number(mcq.correctIndex) <= 3 &&
            isText(mcq.explanation);
        })
      : [],
    shortQuestions: Array.isArray(candidate.shortQuestions)
      ? candidate.shortQuestions.filter((item): item is NonNullable<PracticeSet['shortQuestions']>[number] => {
          if (!item || typeof item !== 'object') return false;
          const question = item as Record<string, unknown>;
          return isText(question.question) && isText(question.modelAnswer);
        })
      : [],
    longQuestions: Array.isArray(candidate.longQuestions)
      ? candidate.longQuestions.filter((item): item is NonNullable<PracticeSet['longQuestions']>[number] => {
          if (!item || typeof item !== 'object') return false;
          const question = item as Record<string, unknown>;
          return isText(question.question) && isText(question.modelAnswer);
        })
      : [],
    definitions: Array.isArray(candidate.definitions)
      ? candidate.definitions.filter((item): item is NonNullable<PracticeSet['definitions']>[number] => {
          if (!item || typeof item !== 'object') return false;
          const definition = item as Record<string, unknown>;
          return isText(definition.term) && isText(definition.definition);
        })
      : [],
  };
}
...
export default function Practice() {
  ...
  const [recordedWrittenSetKey, setRecordedWrittenSetKey] = useState<string | null>(null);
  ...
  useEffect(() => {
    setQuizSelectedKeys(quizChapterTargets.map(targetKey));
  }, [quizChapterTargets]);
  ...
  function completeWrittenSession() {
    if (!practiceSet || currentMcqs.length > 0) return;
    const writtenCount =
      (practiceSet.shortQuestions?.length ?? 0) +
      (practiceSet.longQuestions?.length ?? 0) +
      (practiceSet.definitions?.length ?? 0);
    if (writtenCount === 0) return;
    const attemptSubject = activeAttemptSubject || subject;
    const attemptChapter = activeAttemptChapter || chapter;
    const setKey = `${activeSessionType}|${attemptSubject}|${attemptChapter}|written|${JSON.stringify(practiceSet)}`;
    if (recordedWrittenSetKey === setKey) return;
    ...
    addPracticeAttempt({
      ...
      score: 0,
      total: 0,
      totalQuestions: writtenCount,
      unscored: true,
      revisionReasons: activeSessionType === 'revision' ? activeRevisionReasons : undefined,
      chapters: activeTargets.length
        ? activeTargets.map(({ subject: itemSubject, chapter: itemChapter }) => ({
            subject: itemSubject,
            chapter: itemChapter,
          }))
        : undefined,
    });
    setRecordedWrittenSetKey(setKey);
  }

  async function checkDefinitionAnswer(definition: { term: string; definition: string }) {
    if (!profile) return;
    const definitionTarget = definition as typeof definition & { subject?: string; chapter?: string };
    const definitionSubject = definitionTarget.subject || subject;
    const definitionChapter = definitionTarget.chapter || chapter;
    const key = definitionKey(definitionSubject, definitionChapter, definition.term);
  ...
  const definitionStorageKey = (item: NonNullable<PracticeSet['definitions']>[number]) =>
    definitionKey(item.subject || subject, item.chapter || chapter, item.term);
  const checkedCount = definitionItems.filter(
    (item) => definitionChecks[definitionStorageKey(item)],
  ).length;
  const correctDefinitionCount = definitionItems.filter(
    (item) => definitionChecks[definitionStorageKey(item)]?.correct,
  ).length;
  const currentDefinitionKey = currentDefinition
    ? definitionStorageKey(currentDefinition)
    : '';
      ...
      <div className="space-y-5 px-5 pb-6 pt-8 sm:px-7">
        ...
        {practiceMode === 'quiz' && (
          <Card className="p-4" noTap>
            ...
            <p className="mb-2 text-xs font-bold uppercase tracking-wide text-muted-foreground">
              Chapters ({selectedQuizTargets.length} selected)
            </p>
            <div className="max-h-56 space-y-2 overflow-y-auto rounded-2xl border border-border bg-background p-2">
              {quizChapterTargets.map((target) => {
                const key = targetKey(target);
                const selected = quizSelectedKeys.includes(key);
                return (
                  <button
                    key={key}
                    type="button"
        ...
        {practiceSet && practiceMode !== 'quiz' && (
          <div className="space-y-5">
            ...
            {currentMcqs.length === 0 &&
              ((practiceSet.shortQuestions?.length ?? 0) +
                (practiceSet.longQuestions?.length ?? 0) +
                (practiceSet.definitions?.length ?? 0) > 0) && (
                <Button
                  fullWidth
                  disabled={recordedWrittenSetKey !== null}
                  onClick={completeWrittenSession}
                >
                  <Check className="mr-2 h-4 w-4" />
                  {recordedWrittenSetKey ? 'Session Completed' : 'Complete Written Session'}
                </Button>
              )}
          </div>
        )}
            ...
            <div className="space-y-3">
              {sortedHistory.slice(0, 8).map((attempt) => (
                ...
                >
                  ...
                  <span className={`rounded-full border px-2 py-1 text-xs font-black ${attempt.unscored ? 'border-border bg-secondary text-muted-foreground' : scoreTone(attempt.score, attempt.total)}`}>
                    {attempt.unscored ? 'Done' : `${scorePercent(attempt.score, attempt.total)}%`}
                  </span>
          ...
          >
            ...
            >
              <div className="mb-4 flex items-start justify-between gap-3">
                ...
                <div className={`rounded-2xl border px-3 py-2 text-center ${selectedAttempt.unscored ? 'border-border bg-secondary text-muted-foreground' : scoreTone(selectedAttempt.score, selectedAttempt.total)}`}>
                  <p className="text-2xl font-black">{selectedAttempt.unscored ? 'Done' : `${scorePercent(selectedAttempt.score, selectedAttempt.total)}%`}</p>
                  <p className="text-[10px] font-bold uppercase tracking-wide">
                    {selectedAttempt.unscored ? `${selectedAttempt.totalQuestions ?? 0} written items` : `${selectedAttempt.score}/${selectedAttempt.total}`}
                  </p>
                </div>
              </div>
              ...
              {selectedAttempt.questionsAndAnswers?.length ? (
                ...
                <p className="rounded-2xl bg-secondary p-4 text-sm text-muted-foreground">
                  {selectedAttempt.unscored
                    ? 'This written practice session was completed without an automatic score.'
