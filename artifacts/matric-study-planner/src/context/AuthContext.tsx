...
function setSessionStorageItem(key: string, value: string) {
  try {
    ...
    // Authentication still works for the current page without session storage.
  }
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [isGuest, setIsGuest] = useState(loadGuestState);
    ...
    void getFirebaseServices()
      .then(async (services) => {
        ...
        unsubscribe = onAuthStateChanged(services.auth, (user) => {
          if (user) {
            if (getSessionStorageItem(AUTH_SIGNING_OUT_KEY) === 'true') {
              setLoading(false);
              return;
            }
            ...
            setLocalStorageItem(AUTH_USER_KEY, JSON.stringify(authUser));
            removeLocalStorageItem(AUTH_GUEST_KEY);
          } else if (!getLocalStorageItem(AUTH_GUEST_KEY)) {
            setCurrentUser(null);
  ...
  const signOut = useCallback(async () => {
    setSessionStorageItem(AUTH_SIGNING_OUT_KEY, 'true');
    setCurrentUser(null);
    setIsGuest(false);
    removeLocalStorageItem(AUTH_USER_KEY);
    removeLocalStorageItem(AUTH_GUEST_KEY);
    ...
    window.setTimeout(() => {
      removeSessionStorageItem(AUTH_SIGNING_OUT_KEY);
    }, 1500);
  }, []);
