...
import { normalizeAiSchedule, type AiSchedule } from '@/types/schedule';
...
function computeMode(
  ...
  const isBehindPace = overallProgress < expectedProgress - 20;
...
function normalizeReminderSettings(value: unknown): ReminderSettings {
  ...
  return {
    ...
    time,
    lastShownDate: typeof candidate.lastShownDate === 'string' ? candidate.lastShownDate : undefined,
  };
}

function normalizeEvents(value: unknown): StudyEvent[] {
  if (!Array.isArray(value)) return [];
  return value.filter((item): item is StudyEvent => {
    if (!item || typeof item !== 'object') return false;
    const event = item as Partial<StudyEvent>;
    return typeof event.id === 'string' && typeof event.title === 'string' && Boolean(event.title.trim()) &&
      ...
      (event.type === 'test' || event.type === 'revision' || event.type === 'free' || event.type === 'custom');
  }).slice(-100);
}

function normalizeTutorHistory(value: unknown): TutorChatMessage[] {
  if (!Array.isArray(value)) return [];
  return value.filter((item): item is TutorChatMessage => {
    if (!item || typeof item !== 'object') return false;
    const message = item as Partial<TutorChatMessage>;
...
function normalizePracticeHistory(value: unknown): PracticeAttempt[] {
  if (!Array.isArray(value)) return [];
  return value.filter((item): item is PracticeAttempt => {
    if (!item || typeof item !== 'object') return false;
    const attempt = item as Partial<PracticeAttempt>;
    return typeof attempt.id === 'string' && typeof attempt.subject === 'string' &&
      typeof attempt.chapter === 'string' && typeof attempt.date === 'string' &&
      Number.isFinite(Date.parse(attempt.date)) && Number.isFinite(attempt.score) &&
      Number.isFinite(attempt.total) && Number(attempt.score) >= 0 && Number(attempt.total) >= 0;
  }).slice(0, 30);
}
...
export function AppContextProvider({ children }: { children: ReactNode }) {
  ...
  const [currentDateKey, setCurrentDateKey] = useState(todayStr);
  ...
  useEffect(() => {
    ...
    setStreak(Number.isInteger(streakData.count) && streakData.count > 0 ? streakData.count : 0);
    setLastStudiedDate(typeof streakData.lastDate === 'string' ? streakData.lastDate : null);

    const rawBadges = loadJSON<unknown>(key('matric_badges'), []);
    const badges = Array.isArray(rawBadges)
      ? [...new Set(rawBadges.filter((item): item is string => typeof item === 'string'))]
      : [];
    setEarnedBadges(badges);

    const evts = normalizeEvents(loadJSON<unknown>(key('matric_events'), []));
    setEvents(evts);
    ...
    setScheduleSelectionConfigured(loadJSON<unknown>(key('matric_schedule_selection_configured'), false) === true);

    const storedSchedule = normalizeAiSchedule(loadJSON<unknown>(key('matric_ai_schedule'), null));
    if (storedSchedule) setAiScheduleState(storedSchedule);

    const storedTutorHistory = normalizeTutorHistory(loadJSON<unknown>(key('tutorChatHistory'), []));
    setTutorChatHistoryState(storedTutorHistory);

    const storedPracticeHistory = normalizePracticeHistory(loadJSON<unknown>(key('matric_practice_history'), []));
    setPracticeHistory(storedPracticeHistory);

    const storedReminders = normalizeReminderSettings(loadJSON<unknown>(key('matric_reminder_settings'), DEFAULT_REMINDER_SETTINGS));
    setReminderSettingsState(storedReminders);
  ...
  const setProfile = useCallback((p: Profile) => {
    ...
    setChapterCompletion((prev) => {
      const next = normalizeCompletionForSubjects(nextProfile.subjects, prev);
  ...
  const currentMode = useMemo(
    ...
    [currentDateKey, profile, overallProgress],
  );

  const activeStreak = useMemo(() => {
    if (!lastStudiedDate || streak <= 0) return 0;
    const ageInDays = -daysUntilDateOnly(lastStudiedDate);
    return ageInDays >= 0 && ageInDays <= 1 ? streak : 0;
  }, [currentDateKey, lastStudiedDate, streak]);
  ...
  const toggleChapter = useCallback(
    (subject: string, chapter: string) => {
      setChapterCompletion((prev) => {
        ...
        if (!prevDone) {
          ...
          const subjects = profile?.subjects ?? Object.keys(next);
    ...
    [profile?.subjects, recordStudyActivity, saveUserJSON, unlockBadge],
  );
  ...
  const setAiSchedule = useCallback((s: AiSchedule | null) => {
    const next = s ? normalizeAiSchedule(s) : null;
    setAiScheduleState(next);
    if (next) {
      saveUserJSON('matric_ai_schedule', next);
    } else {
  ...
  const setTutorChatHistory = useCallback((messages: TutorChatMessage[]) => {
    ...
    setTutorChatHistoryState(next);
    saveUserJSON('tutorChatHistory', next);
  }, [saveUserJSON]);
