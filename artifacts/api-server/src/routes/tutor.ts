...
import { sanitizeAvailableTutorSubjects } from "../config/tutorSubjectClassification";
...
router.post("/tutor-chat", runUpload, async (req: Request, res: Response): Promise<void> => {
  ...
  const responseSubject = manualSubject ?? "General";
  ...
  const tutorSystemPrompt = buildSystemPrompt({
    currentMode,
    subject: safeSubject,
  ...
  try {
    if (uploadedFile) {
      ...
      if (kind === "image") {
        const reply = await generateGeminiTutorReply({
          apiKey,
